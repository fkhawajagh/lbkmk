...
- **Documentation updates required.** `docs/integrations/xero.md` best-practice #1 and anti-pattern #12 need revision (done in this PR). `docs/domain-model.md` §8 Q2 needs update (done in this PR). `docs/solution-proposal.md` §3 Flow 3 (Xero write) needs update — deferred to a follow-up docs sweep because the solution proposal is a broader doc that crosses multiple integration boundaries. `docs/project-status.yaml` Phase 1 open question `xero-posting-clearing-account` closes (done in this PR).
...
- **Open risks to validate before implementation:**
  1. **Void/reversal of `RECEIVE` BankTransactions.** Xero has no native "void" for BankTransactions. The reversal strategy (negating line items or posting a counter `SPEND`) needs sandbox validation to confirm it correctly reverses both inventory and revenue recognition.
  2. **Clearing account `Code` availability.** The `BankAccount.Code` field is used to target the clearing account. If LBK's clearing accounts lack user-defined codes, lbkmk needs an `AccountID` lookup fallback via `GET /Accounts`.
  3. **Sales reporting with `RECEIVE` transactions.** Xero's built-in "Sales Overview" report may filter to `ACCREC` Invoices only. This should be validated in a sandbox; if confirmed, the owner needs training on General Ledger / custom reports before go-live.
