...
```
   ...
   │                                                          │
   │   Invoice (ACCREC, AUTHORISED)  ──────►  Inventory Item  │
   ...
   │   Square bank-feed deposit  ───►  the Invoice(s) above   │
   └──────────────────────────────────────────────────────────┘
...
Per `docs/domain-model.md` §6 rule 5, **Xero is the only writer of inventory counts**: lbkmk never PUTs stock levels, only POSTs invoices that cause Xero to decrement stock as a side-effect. Per rule 12, every Xero call is logged in full (request payload, response payload, status) so the system can always answer "what exactly did we send and what did Xero say".
...
| **Invoice** (`InvoiceID` GUID + `InvoiceNumber` string) | The Xero output of one approved Sale Event | `Type=ACCREC` (Accounts Receivable, i.e. "sales invoice"). Carries a `Reference` field — lbkmk uses `lbkmk:<sale_event.id>` here as a **soft idempotency key** that survives even if the Xero `Idempotency-Key` header window has expired. Has its own line items, currency, tax rates, tracking categories. ([invoices][invoices]) |
...
| **CreditNote** (`CreditNoteID` GUID) | Refund handling | Issued against a previously-AUTHORISED invoice when LBK refunds a customer. `Type=ACCRECCREDIT`. Decrements inventory **in reverse** (returns stock to the shelf). lbkmk's v1 defers refunds (`docs/domain-model.md` §8), but the credit-note shape is the right abstraction when it lands. ([credit-notes][credit-notes]) |
...
|---|---|---|
| Create an itemized invoice | `POST /api.xro/2.0/Invoices` with body `{"Type":"ACCREC", "Contact":{"ContactID":"..."}, "Date":"...", "DueDate":"...", "LineAmountTypes":"Exclusive"/"Inclusive"/"NoTax", "LineItems":[...], "Status":"AUTHORISED", "Reference":"lbkmk:<sale_event.id>", "CurrencyCode":"GBP"}` and headers `Idempotency-Key: <uuid-v4>`. **lbkmk's primary write.** ([invoices][invoices]) |
| Create multiple invoices in one call | `POST /Invoices` with body `{"Invoices":[ {...}, {...}, ... ]}` (up to ~50 nodes per request per [api-limits][api-limits]) | Each invoice in the batch independently needs its own `Reference`. Useful for backfill, less useful for steady-state ingestion. |
| Get one invoice | `GET /Invoices/{InvoiceID}` or `/Invoices/{InvoiceNumber}` | Line items + Payments + CreditNotes returned. |
| Get many invoices (incremental sync) | `GET /Invoices?statuses=AUTHORISED,PAID&page=1` with header `If-Modified-Since: <UTC>` | **Paging is recommended** — the Invoices endpoint enforces paging when filtering by Statuses. Up to 100 invoices per page (or `pageSize=250` if needed). ([invoices][invoices]) |
| Search invoices by Reference | `GET /Invoices?where=Reference="lbkmk:<sale_event.id>"` | The lbkmk-side belt-and-braces idempotency check before POSTing: if an invoice already exists with this Reference, do not POST again. Note that `Reference` is in the **optimised filter set** ([invoices][invoices]) so this query is cheap. |
| Void an invoice | `POST /Invoices/{InvoiceID}` with body `{"Status":"VOIDED"}` | Voiding requires the invoice be in `AUTHORISED` or `SUBMITTED` status. A voided invoice reverses the inventory decrement automatically. ([invoices][invoices]) |
...
| Create a credit note (refund) | `POST /CreditNotes` with body `{"Type":"ACCRECCREDIT", "Contact":{...}, "LineItems":[...], "Status":"AUTHORISED", "Reference":"lbkmk-refund:<sale_event.id>"}` | **Deferred to v2** per `docs/domain-model.md` §8. Listed for completeness. ([credit-notes][credit-notes]) |
...
- **Initial backfill** — if LBK has months of pre-existing channel sales to post into Xero, batch them in 50-invoice POSTs (which counts as **1 API call**, not 50) and pace at < 60/minute
- **Concurrent cap of 5** — lbkmk should serialise Xero writes per tenant (single GenServer or per-tenant work queue), not parallelise across invoices. The dashboard's inventory-snapshot pull also competes for this budget
...
- Same key + same payload + same endpoint → returns the cached prior response (no second invoice created)
...
2. **`Reference: "lbkmk:<sale_event.id>"`** field on every invoice — survives indefinitely. Before POSTing, lbkmk can `GET /Invoices?where=Reference="lbkmk:<sale_event.id>"` to confirm absence. After a 5xx, the same Reference + same UUID can be safely re-tried; if Xero already created the invoice on the prior attempt, the cached response is returned.
...
1. **One invoice per Sale Event, posted directly as `AUTHORISED`.** Don't batch unless backfilling. Don't post as `DRAFT` — drafts don't decrement inventory, and the human approval step has already happened in lbkmk's UI. ([invoices][invoices])
...
4. **Pre-fetch `tenantId`, account codes, tracking-category IDs, item codes once at boot** (and on a daily refresh). Caching these prevents per-invoice round-trips to `/Accounts`, `/TrackingCategories`, `/Items` and keeps within the 60/min budget.
...
7. **Pin `CurrencyCode` on every invoice.** Default is the tenant's base currency (GBP for LBK). If Stripe accepts a USD or EUR payment, post the Xero invoice in that currency — Xero handles FX conversion to GBP at the posting date's exchange rate.
...
13. **Treat Items' `QuantityOnHand` as read-only.** Never PUT changes to stock. Stock changes happen only as side-effects of invoices and credit notes. Inventory adjustments (e.g. stock-take corrections) are out of scope for lbkmk and should be done in the Xero UI by the owner. ([tracked-inventory][tracked-inventory])
...
1. **Posting `Status: "DRAFT"` instead of `AUTHORISED`.** Draft invoices do **not** decrement inventory and do **not** show in the bank-feed reconciliation matching algorithm. They sit in Xero's "Awaiting Approval" queue waiting for a human click. lbkmk's whole purpose is to skip that click — post as `AUTHORISED` directly. ([invoices][invoices])

2. **Treating the Xero `Idempotency-Key` window as long-term protection.** The header is the right tool for retry-after-5xx within a short window, but the retention period is not publicly documented. Long retries (e.g. after a Xero maintenance outage that runs hours) may produce duplicate invoices if `Reference`-based dedupe isn't also in place. **Always pre-check `GET /Invoices?where=Reference="lbkmk:<sale_event.id>"`** before POSTing on a retry path where the prior attempt's outcome is unknown. ([idempotency][idempotency])

3. **Letting the refresh token expire after 60 days of dormancy.** Refresh tokens are sliding-window — using them resets the 60-day clock. But a system that doesn't refresh between Friday and the following Wednesday for a long weekend × N is fine; one that goes silent for 60 days requires a human user to re-authorise interactively through the browser. Schedule a heartbeat refresh (e.g. weekly) even if no invoices have been posted, to keep the token alive. ([oauth2-auth-flow][oauth2-auth-flow])
...
12. **Assuming the existing Stripe / Square bank feeds will auto-match against lbkmk's invoices.** Xero's bank-feed matching algorithm uses contact name + amount + date heuristics ([finlert-stripe-reconcile][finlert-stripe-reconcile]). Bank-feed deposits from Stripe are **gross of fees minus net deposit timing windows** — they do not match 1:1 to a single Invoice's gross. The reconciliation pattern that works is: lbkmk posts each Sale's Invoice (decrementing inventory and posting revenue against the Stripe-clearing or umbrella revenue account), then the Stripe bank feed brings in the daily/weekly payout, which the owner reconciles **against the sum of recent invoices** via Xero's "Find & Match" UI. **lbkmk does NOT make this match happen automatically** — it makes the match possible by ensuring the Invoice population is correct. See "Open questions" §1 below — this is `docs/domain-model.md` §8 Q2 and the **single biggest open scope question** on lbkmk.

13. **Posting one Xero Invoice per Stripe-charge that double-counts Squarespace sales.** Squarespace's `order.create` and Stripe's `charge.succeeded` for the same customer are **two halves of one transaction** (`docs/domain-model.md` §4.2 Correlation). lbkmk must post **one** Xero Invoice per Correlation pair, not two. The current model has the Squarespace Sale Event carry the line items (the goods sold) and Stripe carry the payment side; the Xero Invoice mirrors the Squarespace line items, **not** the Stripe gross. Double-invoicing would double inventory decrements and double revenue — and would not be caught by the bank feed reconciliation because the bank feed is a once-per-day net deposit.
...
15. **Forgetting the 50-node-per-batch practical ceiling.** The 10MB request size limit ([api-limits][api-limits]) translates to ~50 invoices per batch request in practice. Bigger batches trigger timeouts even when they're well under 10MB. Backfill code must page at ~50.

16. **Trusting webhook events as the source of truth about invoice state.** Xero webhooks cover only `CREATE` and `UPDATE` on Contact and Invoice — not `Items`, not `BankTransactions`, not `Payments`, not `CreditNotes`. Any reconciliation logic that needs visibility into payments-applied or stock-on-hand changes **must poll** (`GET /Items`, `GET /BankTransactions`). ([webhook-overview][webhook-overview])

## Open questions for lbkmk

1. **Bank-feed reconciliation: Stripe and Square feeds vs lbkmk-posted invoices.** [`docs/domain-model.md` §8 Q2.] LBK almost certainly has Stripe and Square already connected to Xero via Xero's official Stripe direct feed ([stripe-xero-integration][stripe-xero-integration]) and Square Xero integration ([square-xero-app][square-xero-app]). The Stripe feed creates a "virtual bank account" in Xero showing every charge, fee, refund, and payout. Once lbkmk posts an itemized Invoice (which credits a revenue account and creates an AR balance), the question is **how that AR is cleared**: (a) by the owner reconciling each Stripe-feed line manually against the corresponding Invoice via Xero's "Find & Match" UI; (b) by a Xero bank rule that auto-matches by `Reference` or amount; or (c) not at all — leaving Xero with double-counting (revenue from the lbkmk Invoice AND revenue from the bank-feed line if it's auto-categorised as Sales). **Resolution path: confirm with owner how Stripe/Square deposits hit Xero today; review Xero bank rules in place; design lbkmk's posting so the bank-feed deposit clears the AR without duplicating revenue.** This is the **biggest open question on the whole project** — getting it wrong means the books are double-counted or empty.
...
4. **Refund flow design.** [`docs/domain-model.md` §8 question 4.] Refunds are deferred to v2, but the Xero shape (`ACCRECCREDIT` credit notes that decrement-in-reverse) needs to be sketched so v1 doesn't paint into a corner. **Recommendation: when a refund webhook arrives from Stripe / Square / Squarespace / TicketTailor, raise a "needs_resolution" flag on the lbkmk side; the owner manually creates a Credit Note in Xero against the original Invoice. Automation lands in v2.**

5. **Xero plan tier — does LBK have tracked inventory and multi-currency enabled?** Tracked inventory requires at least Standard tier; multi-currency requires Premium (in the UK). If LBK is on Starter or Cashbook, the Item / Invoices approach breaks. **Resolution: owner confirms current Xero plan.**
...
11. **Where do Stripe processing fees live in the Xero books?** The Stripe direct feed brings in fee lines as separate transactions ([stripe-xero-integration][stripe-xero-integration]); lbkmk's Invoice posts gross revenue, leaving the fee to be expensed when the bank-feed line is reconciled. This needs to match how the owner already does it — if the owner has a "Stripe Merchant Fees" expense account, lbkmk doesn't need to mention fees on its Invoices at all (Xero handles them via the bank feed).
...
13. **What happens when the owner manually edits an lbkmk-posted Invoice in Xero?** lbkmk has no way to detect this without webhooks. Without webhooks, the owner's edit silently diverges from lbkmk's view. The mitigation is to flag this as a documented "do not edit lbkmk invoices in Xero" convention plus an occasional reconciliation sweep that compares lbkmk's stored Invoice payload to a fresh `GET /Invoices/<id>`. **Resolution: documented owner convention; v2 webhook-based detection.**
