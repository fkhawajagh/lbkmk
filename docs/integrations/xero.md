...
6. **Pin the inventory model: `IsTrackedAsInventory: true`** on every Item lbkmk creates. Requires Xero Standard tier or higher (sets `InventoryAssetAccountCode` and `COGSAccountCode` on the Item). Tickets should be tracked Items, scoped to the event (Adult Pass — Spring 2026 is a different Item from Adult Pass — Autumn 2026). ([tracked-inventory][tracked-inventory], `docs/domain-model.md` §8 ticket-cap open question)
...
5. ~~**Xero plan tier — does LBK have tracked inventory and multi-currency enabled?**~~ **Resolved.** Decision: upgrade to a tier supporting unlimited invoices and tracked inventory (Standard or higher, ~$42–50/month base). Items are tracked (`IsTrackedAsInventory: true`) per the reopened #53. Multi-currency is moot: sales are GBP-only.
